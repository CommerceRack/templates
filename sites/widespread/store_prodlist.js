...
//				dump("BEGIN _app.ext.store_product.calls.appProductGet");
//				dump(" -> PID: "+obj.pid);
//				dump(" -> obj['withReviews']: "+obj['withReviews']);
				tagObj = $.isEmptyObject(tagObj) ? {} : tagObj; 
				tagObj["datapointer"] = "appProductGet|"+obj.pid; 

//fetchData checks the timestamp, so no need to doublecheck it here unless there's a need to make sure it is newer than what is specified (1 day) in the fetchData function				
				if(_app.model.fetchData(tagObj.datapointer) == false)	{
//					dump(" -> appProductGet not in memory or local. refresh both.");
					r += 1;
					}
...
//callbacks.init need to return either a true or a false, depending on whether or not the file will execute properly based on store account configuration.
		init : {
			onSuccess : function()	{
//				dump('BEGIN _app.ext.store_prodlist.init.onSuccess ');
				return true;  //currently, there are no config or extension dependencies, so just return true. may change later.
//				dump('END _app.ext.store_prodlist.init.onSuccess');
				},
			onError : function()	{
				dump('BEGIN _app.ext.store_prodlist.callbacks.init.onError');
				}
			},
/*
...
//				dump("BEGIN _app.ext.store_prodlist.callbacks.translateTemplate.onSuccess");
//				dump(tagObj);
//				dump(" -> tagObj.datapointer = "+tagObj.datapointer);
//				dump(" -> tagObj.parentID = "+tagObj.parentID+" and $(#"+tagObj.parentID+").length: "+$('#'+tagObj.parentID).length);
				var tmp = _app.data[tagObj.datapointer];
				var pid = _app.data[tagObj.datapointer].pid;
//				dump(" -> typeof _app.data['appReviewsList|'+pid]:"+ typeof _app.data['appReviewsList|'+pid]);
				if(typeof _app.data['appReviewsList|'+pid] == 'object'  && _app.data['appReviewsList|'+pid]['@reviews'].length)	{
//					dump(" -> Item ["+pid+"] has "+_app.data['appReviewsList|'+pid]['@reviews'].length+" review(s)");
...
//					dump($._data($prodlist[0],'events')); //how to see what events are tied to an element. not a supported method.
...
//error needs to clear parent or we end up with orphans (especially in UI finder).
			onError : function(responseData,uuid)	{
				...
				var $product = (responseData._rtag.jqObj instanceof jQuery) ? responseData._rtag.jqObj :  $(_app.u.jqSelector('#',responseData._rtag.parentID));
...
//				dump("BEGIN store_prodlist.renderFormats.productList");
//				dump(" -> data.bindData: "); dump(data.bindData);
...
//				dump('BEGIN _app.ext.store_prodlist.renderFormats.mpPagesAsListItems');
				var o = '';
...
//				dump(" -> ID before any manipulation: "+$tag.attr('id'));
				var pid = data.value;
				var showATC = true;
...
//						dump("BUTTON pushed. $(this).parent().attr('id') = "+$(this).parent().attr('id'));
						$(this).parent().submit();
						event.preventDefault();
						}).text('Add To Cart')
						}
					else	{
//						dump(" -> is choose options.");
...
//				dump("BEGIN store_prodlist.u.setProdlistVars");
//				dump(obj);
				var r = false;
...
//allows for just the mp controls to be turned off but will leave the rest of the header (summary, etc) enabled.
					...
					dump(" -> Missing some required fields for setProdlistVars. requires csv and loadstemplate."); dump(obj);
					r = false;
					}
				return r;
				}, //setProdlistVars
...
//				dump("BEGIN store_prodlist.u.getProductDataForList ["+plObj.parentID+"]");

				Q = Q || 'mutable';
				var numRequests = 0; //# of requests that will b made. what is returned.
				if(plObj && plObj.csv)	{
//					dump(" -> csv defined. length: "+plObj.csv.length);
					var pageCSV = this.getSkusForThisPage(plObj);
					var L = pageCSV.length;
					...
					for(var i = 0; i < L; i += 1)	{
//						dump("Queueing data fetch for "+pageCSV[i]);
						var _tag = {};
						if(plObj.isWizard)	{
...
//				dump("BEGIN store_prodlist.u.buildProductList()");
//				dump(" -> obj: "); dump(obj);
...
//					dump(" -> required parameters exist. Proceed...");
...
//					dump(" -> plObj: "); dump(plObj);
//					dump(" -> obj: "); dump(obj);
					if(obj.useChildAsTemplate)	{
						dump(" -> obj.useChildAsTemplate is true.");
						obj.loadsTemplate = "_"+$tag.attr('id')+"ListItemTemplate";
						if(_app.templates[obj.loadsTemplate])	{
							dump(" -> template already exists");
							//child has already been made into a template. 
							}
						else	{
							dump(" -> template does not exist. create it");
							if($tag.children().length)	{
								dump(" -> tag has a child. create template: "+obj.loadsTemplate);
...
//				dump("BEGIN _app.ext.store_prodlist.u.handleProductList");
//				dump(" -> parent = "+parentID);
...
//				dump("BEGIN _app.ext.store_prodlist.u.mpJumpToPage");
...
//				dump("BEGIN store_product.u.summarizeReviews");
				var L = 0;
				var sum = 0;
				var avg = 0;
